// ✅ purchaseList.js (홍길동 하드코딩 제거 + 안전한 인사말 유지)

document.addEventListener('DOMContentLoaded', () => {
  const greetingElem = document.getElementById('userGreeting');

  // ✅ JSP에서 이미 이름을 출력했다면 그대로 두고,
  // 세션 이름이 비어 있을 경우에만 기본 문구 출력
  if (greetingElem && greetingElem.textContent.trim() === '') {
    const storedName = localStorage.getItem('userName');
    if (storedName) {
      greetingElem.textContent = `${storedName}님 반갑습니다`;
    } else {
      greetingElem.textContent = `회원님 반갑습니다`;
    }
  }
...
function showPage(pageNumber) {
  ...
  document.getElementById('nextBtn').disabled =
    pageNumber === document.querySelectorAll('.pagination-btn[data-page]').length;
}
...
function filterRows(searchTerm = '') {
  const selectedStatus = document.getElementById('statusFilter')
  ...
  rows.forEach(row => {
    ...
    const matchesStatus =
      selectedStatus === '' || (statusSpan && statusSpan.classList.contains(`status-${selectedStatus}`));
